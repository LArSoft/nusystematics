#ifndef NUSYST_RESPONSE_CALCULATORS_TEMPLATE_RESPONSE_BASE_HH_SEEN
#define NUSYST_RESPONSE_CALCULATORS_TEMPLATE_RESPONSE_BASE_HH_SEEN
...
NEW_LARSYST_EXCEPT(no_responses_loaded);
NEW_LARSYST_EXCEPT(incompatible_number_of_bins);

template <size_t NDims, bool Continuous = true>
class TemplateResponseCalculatorBase {

protected:
  std::map<larsyst::paramId_t, std::vector<TSpline3>> SplinedBinResponses;
  std::map<larsyst::paramId_t,
  ...
  void LoadInputHistograms(
      ...
      std::map<std::string, larsyst::paramId_t> const &ParamNames);
...
public:
  ...
  virtual bin_it_t GetBin(larsyst::paramId_t,
  ...
  GetVariation(larsyst::paramId_t, double,
  ...
  double GetVariation(larsyst::paramId_t param, double val,
  ...
  typename std::enable_if<Enable, std::pair<double, double>>::type
      GetValidVariations(
          typename std::enable_if<Enable, larsyst::paramId_t>::type);
  /// Extra template parameters required for SINFAE
  template <bool Enable = Continuous>
...
void TemplateResponseCalculatorBase<NDims, Continuous>::LoadInputHistograms(
  ...
  for (auto const &param_config : provider_config) {
    ...
    larsyst::paramId_t pId = ParamNames.at(pname);
...
TemplateResponseCalculatorBase<NDims, Continuous>::GetVariation(
    larsyst::paramId_t param, double val,
...
TemplateResponseCalculatorBase<NDims, Continuous>::GetValidVariations(
    typename std::enable_if<Enable, larsyst::paramId_t>::type param) {
